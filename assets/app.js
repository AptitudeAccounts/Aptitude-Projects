...
let state = {
  ...
  projects: [], milestones: [], budgetCategories: [], remarks: [], suppliers: [], invoices: [], payments: [], documents: [], folders: MOCK_DATA.folders,
};
...
async function loadAllData() {
  ...
  try {
    ...
    return {
      ...
      folders: data.folders && data.folders.length ? data.folders : MOCK_DATA.folders,
    };
  } catch (e) {
...
function initLogin() {
  ...
  function attemptLogin() {
    try {
      ...
      alert("Something went wrong logging in: " + (err.message || err) + "\n\nPlease screenshot this message.");
    }
  }

  const loginForm = document.getElementById("login-form");
  if (loginForm) loginForm.addEventListener("submit", (e) => e.preventDefault()); // belt-and-braces: never allow a native page reload, even if something above changes
  document.getElementById("login-submit-btn").addEventListener("click", attemptLogin);
  document.getElementById("login-password").addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); attemptLogin(); } });
...
function renderDashboard(el) {
  ...
  el.innerHTML = `
    ...
    <div class="grid two-col" style="margin-top:1.5rem;">
      <div class="card">
        <h2 style="font-size:1rem;margin-bottom:0.9rem;">✅ Recently Achieved Milestones</h2>
        ${recentMilestones.length === 0 ? `<p style="font-size:0.82rem;color:var(--graphite-400);">None marked done yet.</p>` : recentMilestones.map((m) => {
          const proj = state.projects.find((x) => x.id === m.project_id);
          return `<div class="list-row" ${proj ? `data-goto="${proj.id}"` : ""}>
            <div class="left"><span style="font-size:1.1rem;">✅</span>
              <div><p class="name">${escapeHtml(safe(m.name, "Milestone"))}</p><p class="sub">${proj ? proj.name : "Unknown project"} ${m.completed_at ? "· " + formatDate(m.completed_at) : ""}</p></div>
            </div>
          </div>`;
        }).join("")}
      </div>
...
function isAdmin_() {
  return state.user.role === "owner" || state.user.role === "operations";
}

async function handleAddFolder_(el, project) {
  const name = prompt("New folder name:");
  if (!name || !name.trim()) return;
  if (state.folders.some((f) => f.name.toLowerCase() === name.trim().toLowerCase())) {
    showBanner(`A folder named "${name.trim()}" already exists.`);
    return;
  }
  const result = await postToSheet("addFolder", { name: name.trim() });
  if (!result.success) return;
  await refreshData();
  renderDocumentsTab(el, project);
}

async function handleRenameFolder_(el, project, folder) {
  const name = prompt(`Rename "${folder.name}" to:`, folder.name);
  if (!name || !name.trim() || name.trim() === folder.name) return;
  if (state.folders.some((f) => f.id !== folder.id && f.name.toLowerCase() === name.trim().toLowerCase())) {
    showBanner(`A folder named "${name.trim()}" already exists.`);
    return;
  }
  const result = await postToSheet("renameFolder", { folderId: folder.id, name: name.trim() });
  if (!result.success) return;
  if (state.openFolder === folder.name) state.openFolder = name.trim();
  await refreshData();
  renderDocumentsTab(el, project);
}
...
function renderDocumentsTab(el, project) {
  ...
  el.innerHTML = `<div class="card" style="padding:0;">
    <div style="display:flex;justify-content:space-between;align-items:center;padding:1.1rem 1.2rem;border-bottom:1px solid var(--graphite-100);">
      ...
      ${admin ? `<button class="link-btn" id="add-folder-btn">＋ Add Folder</button>` : ""}
    </div>
    <div class="folder-grid" style="padding:1.2rem;">
      ${state.folders.map((f) => {
        const count = state.documents.filter((d) => d.project_id === project.id && d.folder === f.name).length;
        return `<div class="folder-card" data-folder-id="${f.id}">
          ${admin ? `<button class="folder-rename-btn" data-rename-folder-id="${f.id}" title="Rename folder">✏️</button>` : ""}
          <button class="folder-btn" data-open-folder="${escapeHtml(f.name)}"><div style="font-size:1.5rem;">📁</div><p style="font-size:0.78rem;font-weight:500;margin-top:0.5rem;">${escapeHtml(f.name)}</p><p style="font-size:0.7rem;color:var(--graphite-400);margin-top:2px;">${count} file${count === 1 ? "" : "s"}</p></button>
        </div>`;
      }).join("")}
    </div>
  </div>`;
  ...
  if (admin) {
    document.getElementById("add-folder-btn").addEventListener("click", () => handleAddFolder_(el, project));
    el.querySelectorAll("[data-rename-folder-id]").forEach((b) => {
      b.addEventListener("click", (e) => {
        e.stopPropagation();
        const folder = state.folders.find((f) => f.id === b.dataset.renameFolderId);
        if (folder) handleRenameFolder_(el, project, folder);
      });
    });
  }
}
...
function renderFullReportTab(el, project, remaining) {
  ...
  document.getElementById("print-report-btn").addEventListener("click", () => {
    ...
    w.document.write(`<html><head><title>${project.name} — Full Project File</title>
      ...
      <style>
        body{font-family:Arial,sans-serif;padding:32px;color:#141821;}
        .letterhead{display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid #C9A227;padding-bottom:14px;margin-bottom:20px;}
        .letterhead img{height:34px;}
        .letterhead .meta{text-align:right;font-size:11px;color:#5A6579;}
        h1{font-size:22px;margin:0 0 4px;}
        h3{border-bottom:2px solid #C9A227;padding-bottom:6px;}
        table{width:100%;border-collapse:collapse;} td,th{padding:4px 8px;text-align:left;} .right{text-align:right;}
        @media print { .letterhead img { -webkit-print-color-adjust: exact; } }
      </style>
      </head><body>
      <div class="letterhead">
        <img src="assets/logo.jpg" alt="Aptitude Group" onerror="this.style.display='none'" />
        <div class="meta">Aptitude Group · aptitude.ae<br>Generated ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</div>
      </div>
      <h1>${project.name} — Full Project File</h1>${html}</body></html>`);
    w.document.close();
