...
const MOCK_DATA = {
  ...
  invoices: [
    { id: "inv-mock-1", supplier_id: "sup-mock-1", invoice_no: "INV-2291", amount: 120000, invoice_date: "2026-06-01", file_url: "" },
  ],
  payments: [
    { id: "pay-mock-1", invoice_id: "inv-mock-1", amount: 80000, paid_date: "2026-06-10", receipt_url: "" },
  ],
  documents: [],
...
function navigate(route, projectId) {
  state.route = route; state.projectId = projectId || null; state.workspaceTab = "Overview"; state.openFolder = null; state.openSupplierId = null;
...
function renderDashboard(el) {
  ...
  el.innerHTML = `
    <div class="grid kpi-grid">
      ${kpi("Active Projects", active, "graphite")}${kpi("Completed", completed, "good")}${kpi("Total Budget", formatAED(totalBudget), "brass")}
      ${kpi("Total Spent", formatAED(totalSpent), "graphite")}${kpi("Remaining Budget", formatAED(totalBudget - totalSpent), "good")}${kpi("Outstanding Payments", formatAED(totalOutstanding), "bad")}
    </div>
      ...
      <div class="card">
        ...
        ${recentRemarks.length === 0 ? `<p style="font-size:0.82rem;color:var(--graphite-400);">Nothing posted yet.</p>` : recentRemarks.map((r) => {
          ...
          return `<div style="display:flex;gap:0.6rem;font-size:0.85rem;margin-bottom:0.8rem;">
            ...
            <div><p style="margin:0;">${escapeHtml(r.text)}</p><p style="margin:2px 0 0;font-size:0.72rem;color:var(--graphite-400);">${r.author} on ${proj ? proj.name : "?"} · ${r.tagged ? "tagged " + r.tagged : ""}</p></div>
          </div>`;
        }).join("")}
      </div>
    </div>
    <div class="grid two-col" style="margin-top:1.5rem;">
      <div class="card"><h2 style="font-size:1rem;margin-bottom:0.9rem;">Projects Requiring Approval</h2>
        ${p.filter((x) => x.status === "attention" || x.status === "delayed").map((x) => `
          <div class="list-row" data-goto="${x.id}" style="border:1px solid var(--graphite-100);border-radius:0.5rem;margin-bottom:0.5rem;"><div><p class="name">${x.name}</p><p class="sub">Risk: ${x.riskLevel}</p></div>${badge(x.status)}</div>`).join("")}
      </div>
      <div class="card"><h2 style="font-size:1rem;margin-bottom:0.9rem;">Upcoming Openings</h2>
        ${[...p].sort((a, b) => String(a.openingDate).localeCompare(String(b.openingDate))).slice(0, 4).map((x) => `
          <div class="list-row" data-goto="${x.id}" style="border:1px solid var(--graphite-100);border-radius:0.5rem;margin-bottom:0.5rem;"><div><p class="name">${x.name}</p><p class="sub">${x.city}</p></div><p class="mono" style="font-size:0.78rem;color:var(--graphite-600);">${formatDate(x.openingDate)}</p></div>`).join("")}
      </div>
    </div>
  `;
...
function kpi(label, value, accent) {
  ...
  return `<div class="card kpi-card"><div class="kpi-top"><span class="kpi-label">${label}</span><span class="kpi-icon" style="${colors[accent]}">●</span></div><p class="kpi-value mono">${value}</p></div>`;
}
...
function renderOverviewTab(el, project, remaining) {
  ...
  document.getElementById("remark-form").addEventListener("submit", async (e) => {
    ...
    await postToSheet("addRemark", { projectId: project.id, author: state.user.name, role: state.user.role, type, tagged, text });
  });
}
...
function renderSuppliersTab(el, project) {
  ...
  document.getElementById("add-supplier-form").addEventListener("submit", async (e) => {
    ...
    state.suppliers.push({ id: localId, project_id: project.id, name, category, contract_value: contractValue });
...
function renderSupplierList(el, project, suppliers) {
  ...
  listEl.innerHTML = suppliers.length === 0 ? `<p style="font-size:0.85rem;color:var(--graphite-400);">No suppliers added yet.</p>` :
    suppliers.map((s) => {
      ...
      const paidTotal = invoices.reduce((sum, i) => {
        const pays = state.payments.filter((p) => p.invoice_id === i.id);
  ...
  listEl.querySelectorAll("[data-toggle-supplier]").forEach((row) => {
    row.addEventListener("click", () => {
      const sid = row.dataset.toggleSupplier;
      state.openSupplierId = state.openSupplierId === sid ? null : sid;
      renderSupplierList(el, project, suppliers);
...
function renderSupplierDetail(project, supplier, invoices) {
  return `
    <div style="border-top:1px solid var(--graphite-100);margin-top:0.9rem;padding-top:0.9rem;">
...
function attachSupplierDetailHandlers(el, project) {
  el.querySelectorAll(".add-invoice-form").forEach((form) => {
    form.addEventListener("submit", async (e) => {
      ...
      const invoiceNo = form.querySelector(".inv-no").value;
      const amount = form.querySelector(".inv-amount").value;
      const invoiceDate = form.querySelector(".inv-date").value;
  ...
  el.querySelectorAll(".add-payment-form").forEach((form) => {
    form.addEventListener("submit", async (e) => {
      ...
      const amount = form.querySelector(".pay-amount").value;
      const paidDate = form.querySelector(".pay-date").value;
  ...
  if (state.openFolder) {
    ...
    el.innerHTML = `<div class="card" style="padding:0;">
      <div style="display:flex;justify-content:space-between;align-items:center;padding:1.1rem 1.2rem;border-bottom:1px solid var(--graphite-100);">
        <h2 style="font-size:1rem;">📁 ${state.openFolder}</h2>
        <button class="link-btn" id="back-to-folders">← Back to folders</button>
      </div>
      <div style="padding:1.2rem;">
        ...
        ${files.length === 0 ? `<p style="font-size:0.85rem;color:var(--graphite-400);">No files uploaded to this folder yet.</p>` :
          files.map((f) => `<div style="display:flex;justify-content:space-between;padding:0.5rem 0;border-bottom:1px solid var(--graphite-50);font-size:0.85rem;">
            <a href="${f.file_url}" target="_blank" style="color:var(--brass-600);">${escapeHtml(f.file_name)}</a>
            <span style="color:var(--graphite-400);font-size:0.75rem;">${f.uploaded_by} · ${formatDate(f.uploaded_at)}</span>
          </div>`).join("")}
      </div>
    </div>`;
